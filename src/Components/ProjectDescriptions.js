const descriptions =
    {
        entropy1: "Modern flow matching and diffusion models have shown impressive capabilities in the speech and music domains. " +
            "We've reached the point where convincing songs can be generated with a single prompt. However, I would argue that this isn't what most musicians want. " +
            "Instead, they want a tool that allows them to generate and design sound with ease and control. " +
            "The goal of the Entropy Audio project is to create this tool.",
        ...
        entropy2: "The open-source model that first captured my interest was <a href='https://musicgen.com/' target='_blank' rel='noopener noreferrer'>MusicGen from Meta AI [1]</a>, released in June 2023. " +
            ...
            "Although it was a huge step up in open-source at the time, the limitations of an autoregressive approach to audio modeling quickly became apparent while testing and finetuning it (long-range consistency issues and slow inference for long sequences). " +
            "Additionally, the neural audio codec it uses (<a href='https://github.com/facebookresearch/encodec' target='_blank' rel='noopener noreferrer'>Encodec [9]</a>) is good, but not quite at the level needed for professional audio.",
        ...
        entropy3: "In June 2024, Stability AI released <a href='https://huggingface.co/stabilityai/stable-audio-open-1.0' target='_blank' rel='noopener noreferrer'>Stable Audio Open (SAO) [4]</a>. " +
            ...
            "In my opinion, SAO is superior to MusicGen in both audio quality and efficiency. " +
        ...
        entropy4: "Model architectures have more or less converged to the DiT recipe, but the data bottleneck still remains. " +
            ...
            "For generating descriptions, I used both proprietary LLM APIs and local LLMs (powered by vLLM and a 5090). " +
            "Each audio sample in the dataset is paired with a JSON file containing a common schema of conditioning fields. " +
        ...
        entropy5: "For my training runs, I swapped out the original text encoder used with SAO for T5Gemma. " +
            ...
            "I created trainer classes for diffusion/flow matching, RL (DDPO, with GRPO advantages) [10], and preference alignment (DPO) [11]. " +
            "To evaluate, some metrics I used were CLAP score, FAD, cosine similarity, frequency distribution, and my own ears to listen to examples produced during training. " +
            ...
            "Finally, to monitor my training runs I used Weights & Biases.",
        ...
        entropy6: "For the Entropy Audio website, I wrote the frontend in Angular and the backend in Python. " +
            ...
            "My goal with the UI's design was to make something that felt like a mix between a digital synthesizer and Claude, with a transparent data flywheel built into the workflow. " +
